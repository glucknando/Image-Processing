        ...
        if self.results.multi_face_landmarks:
            ...
            for faceLms in self.results.multi_face_landmarks:
                ...
                for id,lm in enumerate(faceLms.landmark):
                    #print(lm)
                    ih,iw,ic=img.shape
                    x,y=int(lm.x*iw),int(lm.y*ih)        #x,y v pixelexh
                    #print(id,x,y)                       #id je cislo bodu na xixhte
                    #cv2.putText(img,str(id),(x,y),cv2.FONT_HERSHEY_PLAIN,0.6,(0,255,0),1) # ke kazdemu lm cislo vytiskne
                    face.append([x,y])
                faces.append(face)
        return img,faces
def main():
    cap=cv2.VideoCapture(0)     #"FaceVideos/Video2.mp4"
    #pTime=0
